...
bot.onText(/\/stats/, (msg) => {
  if (msg.chat.id === ADMIN_ID) {
    bot.sendMessage(msg.chat.id,
      `📊 *إحصائيات البوت*\n\n👥 عدد المستخدمين منذ تشغيل البوت: *${users.size}*`,
      { parse_mode: 'Markdown' }
    );
  } else {
...
bot.onText(/\/myid/, (msg) => {
  bot.sendMessage(msg.chat.id, `🆔 ID: \`${msg.chat.id}\``, { parse_mode: 'Markdown' });
});
...
bot.on('message', (msg) => {
  ...
  if (text === '🎁 بونص ترحيبي') {
    bot.sendMessage(chatId, '🎁 *اختر الشركة:*', { parse_mode: 'Markdown', reply_markup: bonusMenu });
  } else if (text === '🔥 عروض') {
    bot.sendMessage(chatId, '🔥 *اختر العرض:*', { parse_mode: 'Markdown', reply_markup: offersMenu });
  } else if (text === '💰 مواقع وبوتات الربح المضمون') {
    bot.sendMessage(chatId, '💰 *اختر:*', { parse_mode: 'Markdown', reply_markup: profitMenu });
  } else if (text === '💳 حسابات تمويل مجانية') {
    bot.sendMessage(chatId, '💳 *اختر الحساب:*', { parse_mode: 'Markdown', reply_markup: fundingMenu });
  }
});
